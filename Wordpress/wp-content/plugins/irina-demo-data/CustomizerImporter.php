	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new \WP_Error(
				...
				sprintf(
					esc_html__( 'Error: The customizer import file is missing! File path: %s', 'novaworks' ),
					$import_file_path
				)
			);
		}
		...
		// Get the upload data.
		$raw = Novaworks_Importer::data_from_file( $import_file_path );
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'novaworks' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new \WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'Error: The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'novaworks' )
			);
		}

		// Import images.
		if ( apply_filters( 'Novaworks_Importer/customizer_import_images', true ) ) {
		...
		$use_wp_customize_save_hooks = apply_filters( 'Novaworks_Importer/enable_wp_customize_save_hooks', false );
